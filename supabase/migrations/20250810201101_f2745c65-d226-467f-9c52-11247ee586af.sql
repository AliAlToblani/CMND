...
-- 1) Update payment schedule function to support one_time and correct boundaries
  ...
  IF NEW.payment_frequency = 'one_time' THEN
    IF COALESCE(NEW.value, 0) > 0 THEN
      INSERT INTO public.payments (
        ...
        NEW.id, NEW.customer_id, COALESCE(NEW.value, 0), NEW.start_date::DATE, 'one_time', 'pending'
      );
    END IF;
    RETURN NEW;
  END IF;
...
-- 2) Create trigger to auto-generate schedule on insert/update
DROP TRIGGER IF EXISTS trg_generate_payment_schedule_after_ins_upd ON public.contracts;

CREATE TRIGGER trg_generate_payment_schedule_after_ins_upd
AFTER INSERT OR UPDATE ON public.contracts
FOR EACH ROW EXECUTE FUNCTION public.generate_payment_schedule();

-- 3) Set all current contracts to annual (as requested)
UPDATE public.contracts
SET payment_frequency = 'annual';

-- 4) Helpful indexes for next-payment lookups
CREATE INDEX IF NOT EXISTS idx_payments_customer_due_status
  ON public.payments (customer_id, status, due_date);

CREATE INDEX IF NOT EXISTS idx_payments_due_status
  ON public.payments (status, due_date);

-- 5) Backfill schedules for existing contracts by touching rows to fire the trigger
UPDATE public.contracts
SET updated_at = now();
