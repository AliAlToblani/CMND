-- First, check existing payment types and fix the constraint
-- Delete existing incorrect payments and regenerate them
DELETE FROM public.payments WHERE contract_id IN (
  '235555b3-fa14-49d2-a6b5-99e9081dba57', -- Ahlia
  '047130d2-cb6e-4e5d-a7c8-5d2bd9f02dc0', -- Provin Contract 1
  '6e6c0931-d23f-4223-9115-af3b2189572b'  -- Provin Contract 2
);

-- Update the payment generation function to use allowed payment types
...
BEGIN
  ...
  -- Handle annual contracts: one upfront payment (setup + full annual) - use 'recurring' type
  IF NEW.payment_frequency = 'annual' THEN
    IF (setup_fee_amount + COALESCE(NEW.annual_rate, NEW.value, 0)) > 0 THEN
      INSERT INTO public.payments (
        ...
        NEW.id, NEW.customer_id, 
        setup_fee_amount + COALESCE(NEW.annual_rate, NEW.value, 0), 
        NEW.start_date::DATE, 'recurring', 'pending'
      );
    END IF;
    RETURN NEW;
  END IF;
...
-- Trigger regeneration by updating the contracts
UPDATE contracts 
SET updated_at = NOW()
WHERE id IN (
  '235555b3-fa14-49d2-a6b5-99e9081dba57', -- Ahlia
